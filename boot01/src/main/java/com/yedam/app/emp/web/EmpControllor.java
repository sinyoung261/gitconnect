...
public class EmpControllor {
	...
	@Autowired
	public EmpControllor(EmpService empService) {
	...
	//전체조회 : GET
	//1)URL + METHOD
	@GetMapping("empList")  //Model = Request + Response, 자동으로 분류시켜준다
	...
	@GetMapping("empInfo") //empInfo?key(employeeId)=value
	public String empInfo(EmpVO empVO, Model model) {
		//2) Service
		EmpVO findVO = empService.findEmpInfo(empVO);
		//2-1) Service의 결과를 View에 전달
		model.addAttribute("emp", findVO); //화면 출력 코드
		//3) View
		return "emp/info";
		// prifix : classpath:/templates/
		// suffix : .html
	...
	public Map<String, Object>
		empUpdateAJAXJSON(@RequestBody EmpVO empVO){ //응답
